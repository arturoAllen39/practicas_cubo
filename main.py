...
from controls import create_controls_window, read_controls, save_config, update_help_display, render_controls
...
<<<<<<< HEAD
        # Background Subtractor (importar desde detectors2)
        from detectors import BackgroundSubtractor
        self.bg_subtractor = BackgroundSubtractor(
            method=params.get('bg_method', 'MOG2'),
            enabled=params.get('use_bg_subtraction', True)
        )
...
<<<<<<< HEAD
            mode_names = ['CIRCLES', 'SHADOWS', 'OPTICAL_FLOW', 'BLOB_TRACKING', 'DARK_REGION']
=======
...
<<<<<<< HEAD
        # Aplicar background subtraction con filtros configurables
        processed_frame, bg_debug = self.bg_subtractor.process(frame, self.params)
        
        # Guardar imágenes de debug de preprocesamiento
        if bg_debug:
            self.debug_images.update(bg_debug)
        
        # Detectar con el método actual (usar frame preprocesado)
        detections = self.detector.detect(processed_frame, self.params)
        
        # Copiar imágenes de debug del detector
        self.debug_images.update(self.detector.debug_images)
=======
...
<<<<<<< HEAD
    ...
    # ----------------------------------------------------------
    print("=" * 60)
    print("  CUBO NEGRO — SELECCIÓN DE FUENTE DE VIDEO")
    print("=" * 60)
    print("  Elige una opción:")
    print("  [1] Cámara en vivo")
    print("  [2] Cargar video pregrabado")
    print("=" * 60)
    ...
    if use_camera:
        print("\n  ✓ Usando cámara en vivo")
        cap = cv2.VideoCapture(0)
        if not cap.isOpened():
            print("ERROR: No se pudo abrir la cámara")
            return
    else:
        print("\n  📁 Selecciona el video a cargar")
        print("  Ingresa la ruta del video (ejemplo: video.mp4)")
        video_path = input("  Ruta: ").strip()
    ...
    params = {
        # Modo de detección (0=Circles, 1=Shadows, 2=OpticalFlow, 3=BlobTracking)
        'detection_mode':        0,
        
        # Preprocesamiento
        'use_bg_subtraction':  True,   # Background subtraction por defecto
        'bg_method':          'MOG2',  # 'MOG2' o 'KNN'
        'bg_learning_rate':    0.001,  # Learning rate bajo (no absorbe animaciones rápido)
        'bg_threshold':        200,    # Threshold para limpiar máscara
        'remove_shadows':      True,   # Eliminar sombras (valor 127)
        'erode_kernel':        3,      # Tamaño kernel erosión
        'erode_iterations':    1,      # Iteraciones erosión
        'dilate_kernel':       5,      # Tamaño kernel dilatación
        'dilate_iterations':   2,      # Iteraciones dilatación
        'closing_kernel':      7,      # Tamaño kernel closing
        'opening_kernel':      5,      # Tamaño kernel opening
        'use_closing':         True,   # Aplicar closing
        'use_opening':         True,   # Aplicar opening
        
        # Dark Region Detector (modo 4)
        'dark_max_value':      80,     # Máximo brillo para considerar "oscuro"
        'dark_erode':          7,      # Erosión agresiva (elimina círculos)
        'dark_dilate':         11,     # Dilatación (recupera personas)
        'dark_closing':        15,     # Closing (cierra huecos)
        'max_circularity':     0.85,   # Rechazar círculos (0-1)
        'min_persistence_frames': 3,   # Frames mínimos para aceptar blob
...
<<<<<<< HEAD
    # Si es video, congelar el primer frame para calibración
    if not use_camera:
        ...
        if not ret:
            ...
            cap.release()
            return
        print("  📸 Primer frame congelado para calibración")
    
    while True:
        if use_camera:
            # Cámara: leer frame en cada iteración
            ret, frame = cap.read()
            if not ret:
                break
        else:
            # Video: usar frame congelado
...
>>>>>>> d1c69e2ed2717a018c58a372c588256fb29e2b7b
    ...
    print("    2: OPTICAL_FLOW  (vectores de movimiento)")
    print("    3: BLOB_TRACKING (del paper - área+color+centroide)")
<<<<<<< HEAD
    print("    4: DARK_REGION   (oscuridad - RECOMENDADO para Cubo Negro)")
=======
>>>>>>> d1c69e2ed2717a018c58a372c588256fb29e2b7b
    print("  Teclas:")
...
>>>>>>> d1c69e2ed2717a018c58a372c588256fb29e2b7b
        ...
        elif current_mode == 2:  # OPTICAL_FLOW
        ...
        elif current_mode == 3:  # BLOB_TRACKING
...
<<<<<<< HEAD
        ...
        # Si es video y llegó al final, reiniciar
        if not ret:
            if not use_camera:
                print("\n  🔄 Video terminado, reiniciando...")
                cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
                ret, frame = cap.read()
                if not ret:
                    print("  ❌ Error al reiniciar el video")
                    break
            else:
...
<<<<<<< HEAD
            mode_names = ['CIRCLES', 'SHADOWS', 'OPTICAL_FLOW', 'BLOB_TRACKING', 'DARK_REGION']
            from controls import _st
            new_mode = (_st["vals"].get("detection_mode", 0) + 1) % 5
=======
            mode_names = ['CIRCLES', 'SHADOWS', 'OPTICAL_FLOW', 'BLOB_TRACKING']
            from controls2 import _st
            new_mode = (_st["vals"].get("detection_mode", 0) + 1) % 4
>>>>>>> d1c69e2ed2717a018c58a372c588256fb29e2b7b
